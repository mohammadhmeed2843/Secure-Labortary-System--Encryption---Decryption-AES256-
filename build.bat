...
for /d %%D in ("%PROJECT_DIR%.runtime\jdk-24*") do (
    if exist "%%~fD\bin\javac.exe" set "JAVA_HOME=%%~fD"
)

if not defined JAVA_HOME if exist "C:\Program Files\Eclipse Adoptium\jdk-24*\bin\javac.exe" (
    for /d %%D in ("C:\Program Files\Eclipse Adoptium\jdk-24*") do set "JAVA_HOME=%%~fD"
)

if not defined JAVA_HOME if exist "C:\Program Files\Java\jdk-24\bin\javac.exe" (
