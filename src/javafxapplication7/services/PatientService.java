...
public final class PatientService {
    ...
    /** Insert or update a patient record (called inside FileService's upload transaction). */
    ...
    /** All patients ordered by last name, first name. */
    ...
    /** Look up a single patient by patient number. Returns null if not found. */
    public static Patient findByNumber(String patientNumber) throws Exception {
        ...
        try (Connection conn = DB.connect();
            ...
            Date d = rs.getDate("dob");
            return new Patient(rs.getString("patient_number"), rs.getString("first_name"),
                    rs.getString("last_name"), d != null ? d.toLocalDate() : null);
        }
    }

    /** Case-insensitive search by name or patient number. */
    public static List<Patient> search(String query, int limit) throws Exception {
        ...
        try (Connection conn = DB.connect();
            ...
            while (rs.next()) {
                Date d = rs.getDate("dob");
                out.add(new Patient(rs.getString("patient_number"), rs.getString("first_name"),
                        rs.getString("last_name"), d != null ? d.toLocalDate() : null));
            }
        }
        return out;
