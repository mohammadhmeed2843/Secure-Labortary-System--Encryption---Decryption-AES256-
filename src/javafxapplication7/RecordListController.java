...
/**
 * Records overview screen for ADMIN and TECHNICIAN roles.
 * Shows all medical records with status badges, search filtering,
 * and per-row export actions (ADMIN only).
 */
public class RecordListController {
    ...
    private void renderRecords(List<FileRecord> records) {
        ...
        for (FileRecord r : records) {
            recordsContainer.getChildren().add(buildRow(r, canExport));
        }
    }
    ...
    private HBox buildRow(FileRecord r, boolean canExport) {
        ...
        // Right: status badge + optional export
        ...
        Thread t = new Thread(() -> {
            try {
                ...
                javafx.application.Platform.runLater(() -> {
                    ...
                    loadRecords(); // refresh status badges
                });
            } catch (Exception ex) {
                javafx.application.Platform.runLater(() -> {
                    btn.setDisable(false);
    ...
    private static String nullStr(String s, String def) { return (s != null && !s.isBlank()) ? s : def; }
