...
public class PatientFormController {
    ...
    public void initialize() {
        ...
        // Read encrypted file from shared store (set by openEncryptionController)
        if (CryptoStore.hasEncryptedFile()) {
            encryptedFile = CryptoStore.getEncryptedFile();
            fileNameLabel.setText("Encrypted PDF (" + encryptedFile.length + " bytes)");
        } else {
            fileNameLabel.setText("(none — go back to Encrypt first)");
        }
    }
    ...
    private void handleSave(ActionEvent event) {
        ...
        if (encryptedFile.length > 1024 * 1024 * 10) {
            showAlert(Alert.AlertType.ERROR, "Encrypted file exceeds the 10 MB limit.");
            return;
        }
        ...
        try (Connection conn = DatabaseConnection.connect()) {
            ...
            conn.commit();
            showAlert(Alert.AlertType.INFORMATION, "Patient data and encrypted file saved successfully.");

            CryptoStore.clearAll();
    ...
    private int insertOrGetId(Connection conn, String table, String idColumn, String nameColumn, String value) throws SQLException {
        ...
        throw new SQLException("Unable to insert or retrieve ID for: " + value);
    }
