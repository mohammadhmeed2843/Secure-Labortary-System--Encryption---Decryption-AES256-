...
import javafxapplication7.services.AdminService;
...
public class AdminUsersController {
    ...
    private void handleCreateUser() {
        ...
        catch (IllegalArgumentException e) { showCreateStatus("Invalid role selected.", true); return; }

        Task<Void> task = new Task<>() {
            @Override protected Void call() throws Exception {
                AdminService.createUser(username, password, role, fullName);
                AuditService.logCurrent(AuditService.CREATE_USER, "user", username, "role=" + roleStr);
                return null;
            }
        };
        task.setOnSucceeded(e -> javafx.application.Platform.runLater(() -> {
            showCreateStatus("User '" + username + "' created.", false);
            newUsernameField.clear(); newFullNameField.clear();
            newPasswordField.clear(); newRoleBox.setValue(null);
            loadUsers();
        }));
    ...
    private void loadUsers() {
        Task<List<User>> task = new Task<>() {
            @Override protected List<User> call() throws Exception { return AdminService.listAllUsers(); }
        };
        task.setOnSucceeded(e -> javafx.application.Platform.runLater(() -> buildUserList(task.getValue())));
        task.setOnFailed(e -> javafx.application.Platform.runLater(() ->
    ...
    private void buildUserList(List<User> users) {
        ...
        if (users.isEmpty()) userListBox.getChildren().add(new Label("No users found."));
    }
    ...
    private void handleToggleActive(int userId, boolean currentlyActive) {
        Task<Void> task = new Task<>() {
            @Override protected Void call() throws Exception {
                ...
                String action = currentlyActive ? AuditService.DEACTIVATE_USER : AuditService.ACTIVATE_USER;
    ...
    private void handleResetPassword() {
        ...
        Task<Void> task = new Task<>() {
            @Override protected Void call() throws Exception {
                ...
                AuditService.logCurrent(AuditService.RESET_PASSWORD, "user", String.valueOf(uid), null);
                return null;
            }
        };
        task.setOnSucceeded(e -> javafx.application.Platform.runLater(() -> {
